...
# --- SAFE INITIALIZATION ---
# Initialize global variables as empty/None first
# We will load them properly inside start_surveillance()
ml_model = None
BANNED_IPS = set()
rate_limiter = RateLimiter(limit=RATE_LIMIT_MAX, window=RATE_LIMIT_WINDOW)

def load_brain():
    """Loads the model safely. Returns True if successful."""
    global ml_model
    print("--- 🔄 Initializing AutoNet AI ---")
    try:
        ml_model = joblib.load(MODEL_PATH)
...
def start_surveillance():
    # LOAD THE BRAIN HERE (Runtime Only)
    if not load_brain():
        exit(1)
